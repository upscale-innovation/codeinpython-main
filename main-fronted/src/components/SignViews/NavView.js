...
const NavView = () => {
 ...
 let user = useSelector(isTokenAvailable)
  ...
  return (
    <div className="p-2 border border-b-0 border-gray-400 shadow-md flex space-x-4 justify-center items-center">
      <MenuIcon className="h-5 w-5  text-gray-500" />
      <p className="text-[#f5a425] font-semibold text-lg whitespace-nowrap">
        GRAD SCHOOL
      </p>
      ...
      <div className=" max-w-screen-sm w-full border border-gray-400 items-center px-2 hidden md:flex rounded-sm">
        ...
        <input placeholder="Search.. " className=" text-sm p-2" />
      </div>
      {!user ? ( <button className=" py-1 px-2 text-sm border-[#f5a425]  border-2 rounded-md cursor-pointer hover:opacity-70 whitespace-nowrap">
        Sign Up
