...
const navigate=useNavigate()
  ...
  const onSubmit = async(data) => {console.log(data)
    const login= await axios.post("http://localhost:8000/login",{country_code:data.countryCode,mobile_number:data.mobileNumber,password:data.password})
    ...
    <div className=" max-w-xs  md:max-w-sm w-screen border p-4">
      <form className="flex flex-col " onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="flex justify-between pt-4 items-center ">
          ...
          <p className=" text-xs text-yellowBase">Forgot password ?</p>
        </div>
        <input
          type="password"
