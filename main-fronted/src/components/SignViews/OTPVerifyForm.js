...
const [errorStatus,setErrorStatus]=useState("")
  ...
  const onSubmit = async(data) => {
    const login= await axios.post("http://localhost:8000/otp_verify",{country_code:data.countryCode,mobile_number:data.mobileNumber,otp:data.otp}).catch(error => setErrorStatus(JSON.parse(error.request.responseText).message))
  ...
  return (
    ...
    <form className="flex flex-col " onSubmit={handleSubmit(onSubmit)}>
      ...
      <button className=" bg-yellowBase text-white py-2 mt-4">Recover a temporary session</button>
    </form>
